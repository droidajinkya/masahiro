...
class ScanController extends ChangeNotifier {
  ...
  void startBackgroundListeners(void Function(bool enabled) onAutoFlashChange) {
    _lightSubscription ??= _ambientLightService.lowLightStream().listen((isLowLight) {
      if (autoFlashEnabled && isLowLight && !flashOn) {
        flashOn = true;
        onAutoFlashChange(true);
        notifyListeners();
      }
    });

    _connectivitySubscription ??=
        _connectivityService.statusStream.listen((_) => _repository.retryQueuedEvents());
  }
